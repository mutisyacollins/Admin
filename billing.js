...
function filterBills() {
  ...
  const filtered = allBills.filter(b => {
    const combined = [b.customerName, b.amount, b.dueDate, b.paymentStatus, b.status, b.description].join(' ').toLowerCase();
...
async function saveBill() {
  ...
  if (amount <= 0)              { errorEl.textContent = 'Amount must be greater than 0.'; return; }

  saveBtn.disabled    = true;
  saveBtn.textContent = 'Saving…';

  try {
    const data = { customerName, amount, dueDate, paymentStatus, description, updatedAt: firebase.firestore.FieldValue.serverTimestamp() };
...
async function markBillPaid(id) {
  ...
  try {
    ...
    await fetchBills();
  } catch { showToast('Failed to update bill.', 'error'); }
}
